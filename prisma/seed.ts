...
async function main() {
  ...
  // ④ 交付归档
  const dl1 = await prisma.deliveryRecord.create({ data: { deliveryNo: "DL-2026-0715-01", vehicleId: "VH-7E002", projectId: "PRJ-2026-ZX-031", type: "normal", status: "delivered", packageSummary: "需求单、方案V2.1冻结版、物料清单5项、报工记录8条、质量检验2次、问题关闭3项", customerName: "试验部 · 陈工", signOffBy: "陈工", signOffAt: new Date("2026-07-15T16:30:00"), toccSynced: true } });
  await prisma.deliveryRecord.create({ data: { deliveryNo: "DL-2026-0720-01", vehicleId: "VH-7E001", projectId: "PRJ-2026-SM-017", type: "normal", status: "pending_acceptance", packageSummary: "需求单、方案V3.0冻结版、物料清单5项、报工记录4条、质量检验1次、问题2项待关闭", customerName: "智驾验证部 · 刘工", remainingIssues: "QI-2026-0718-03 整改中；QI-2026-0717-11 待复验" } });
  await prisma.returnRepair.create({ data: { repairNo: "RR-2026-0716-01", deliveryId: dl1.id, vehicleId: "VH-7E002", reason: "试验反馈：线束接插件松动", description: "客户试验 200km 后发现激光雷达线束接插件有松动迹象，需回厂复检并加固", status: "in_progress", assignee: "总装一班" } });

  // ⑤ 方案评审深化
  await prisma.solutionSection.createMany({ data: [
  ...
  // ⑦ 生产异常 + 返工
  await prisma.productionException.createMany({ data: [
  ...
  await prisma.reworkTask.createMany({ data: [
  ...
  // ⑧ 需求承接
  await prisma.demandRequest.createMany({ data: [
  ...
  // ⑩ 拆换件
  await prisma.partContainer.createMany({ data: [
